...
ZEND_BEGIN_ARG_INFO_EX(arginfo_blake_file, 0, 0, 2)
...
const zend_function_entry blake_functions[] = {
    ...
    PHP_FE(blake_file,   arginfo_blake_file)
...
/* {{{ Do the hash!
Handle parameters for blake and blake_file functions */
static void _php_blake_hash(INTERNAL_FUNCTION_PARAMETERS, int isfilename) {
    ...
    //Get the stream if the first parameter is filename
    if (isfilename) {
	...
	if (isfilename) {
		...
		while ((n = php_stream_read(stream, buf, sizeof(buf))) > 0) {
			_php_blake_update(state, (unsigned char *) buf, n * 8);
		}
...
Generate blake hash */
PHP_FUNCTION(blake) {
    _php_blake_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ proto string blake_file(string filename, int type[, string salt[, bool raw_output = false]])
Generate blake hash using filename */
PHP_FUNCTION(blake_file) {
    _php_blake_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
PHP_FUNCTION(blake_init) {
    ...
    ZEND_REGISTER_RESOURCE(return_value, state, php_blake_le_hashstate);
}
/* }}} */
...
PHP_FUNCTION(blake_update) {
    ...
    ZEND_FETCH_RESOURCE(state, php_hash_state*, &zhash, -1, PHP_BLAKE_RES_NAME, php_blake_le_hashstate);
...
/* {{{ dtor
*/
static void php_hash_state_dtor(zend_rsrc_list_entry *rsrc TSRMLS_DC)
{
